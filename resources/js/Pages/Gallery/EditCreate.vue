<template>
    <AppLayout :title="form.id ? 'Bild bearbeiten' : 'Neues Bild anlegen'">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ form.id ? 'Bild bearbeiten' : 'Bild hochladen' }}
            </h2>
            <p class="mt-1 max-w-2xl text-sm text-gray-500">
                {{ form.id ? 'Bearbeite das Bild' : 'Lade ein neues Bild hoch.' }}
            </p>
        </template>
                            ...
                            <div>
                                <div class="space-y-8 sm:space-y-5">
                                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:pt-5">
                                        <v-input id="name" v-model="form.name" :error="form.errors.name"
                                            label="Titel" />
                                    </div>
...
// Bild-Vorschau
const image_preview = ref(props.gallery?.media[0]?.original_url || null);
...
const save = () => {
    ...
    if (form.id)
        router.post(route('dashboard.gallery.update', form.id), { _method: 'put', ...form })
    else
